createPasswordButton.on('click', function(e){
    if(passwordForgetPassword.val().length >= 8){
        if(confirmPasswordForgetPassword.val() === passwordForgetPassword.val()){

            createPassword();

            passwordForgetPassword.removeClass('is-invalid').addClass('is-valid');
            confirmPasswordForgetPassword.removeClass('is-invalid').addClass('is-valid');
        }else{
            confirmPasswordForgetPassword.addClass('is-invalid');
            $('#password-error-message').html("Password not matching");
        }
    }else if(passwordForgetPassword.val() === "" || confirmPasswordForgetPassword.val() === ""){
